    ...
    def init_sockets(self):
        ...
        try:
            ...
            self._gui_command_sock.bind(
                (config.CONTROLLER_GUI_LISTEN_IP, config.CONTROLLER_GUI_LISTEN_PORT))
            self._gui_command_sock.settimeout(config.SOCKET_TIMEOUT)
            print(
                f"GUI Command UDP server bound to {config.CONTROLLER_GUI_LISTEN_IP}:{config.CONTROLLER_GUI_LISTEN_PORT}")
